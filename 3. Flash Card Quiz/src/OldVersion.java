...
public class OldVersion extends JFrame{
...
// frame constructor
        ...
        problemLabel.setFont(new Font("Comic Sans MS",
                Font.PLAIN, 48));problemLabel.setHorizontalAlignment(SwingConstants.CENTER);
        gridConstraints = new GridBagConstraints();
        gridConstraints.gridx = 0;
        ...
        UIManager.put("TitledBorder.font", new Font("Arial",
                Font.BOLD, 14));
        typePanel.setPreferredSize(new Dimension(130, 130));
        typePanel.setBorder(BorderFactory.createTitledBorder("Type:"));
        typePanel.setBackground(lightBlue);typePanel.setLayout(new GridBagLayout());
        gridConstraints = new GridBagConstraints();
        gridConstraints.gridx = 0;
        ...
        typeCheckBox[0].setSelected(true);factorPanel.setPreferredSize(new Dimension(130, 130));
        ...
        gridConstraints.anchor = GridBagConstraints.NORTH;getContentPane().add(timerPanel, gridConstraints);
        for (int i = 0; i < 3; i++){
        ...
        timerTextField.setForeground(Color.RED);timerTextField.setHorizontalAlignment(SwingConstants.CENTER);
...
// if all boxes unchecked, recheck last clicked box
    ...
    private void timerScrollBarAdjustmentValueChanged(AdjustmentEvent e){
        timerTextField.setText(getTime(timerScrollBar.getValue() * 30));
    }
    private void startButtonActionPerformed(ActionEvent e){
        int score;
        String message = "";if (startButton.getText().equals("Start Practice")){
    ...
    else
    {
        ...
        if (timerRadioButton[2].isSelected())
            timerScrollBar.setEnabled(true);
        problemsTimer.stop();startButton.setText( "Start Practice");
        exitButton.setEnabled(true);
        problemLabel.setText("");
        if (numberTried > 0){
            score = (int)(100 * (double) (numberCorrect) /
                    numberTried);
            message = "Problems Tried: " +
            ...
            else
            {
                ...
                message += "Time Per Problem: " + new DecimalFormat("0.00").format((double) (problemTime) /
                        numberTried) + " sec";
            }
...
// only allow number keys
        if (e.getKeyChar() >= '0' && e.getKeyChar() <= '9')
        {
            ...
            else
            {
                numberTried++;
// check answer
                if (Integer.valueOf(yourAnswer).intValue() ==
                        correctAnswer)
                {
                    numberCorrect++;}
...
// Subtraction
            ...
            {// Multiplication
                ...
                factor = getFactor(3);
                correctAnswer = number * factor;
                problem = String.valueOf(number) + " x " +
                        String.valueOf(factor) + " = ";
            }
...
//random
    ...
    private String getTime(int s){
        ...
        String ms, ss;
        min = (int) (s / 60);
        sec = s - 60 * min;ms = String.valueOf(min);
